...
export const PackageCard: React.FC<{ pkg: NeoPackage; i: number; user: any; onOpenAuth: () => void; onCheckout: (pkg: NeoPackage) => void }> = ({ pkg, i, user, onOpenAuth, onCheckout }) => {
    return (
        <ScrollReveal delay={i * 100}>
            <div className={`group relative p-8 md:p-12 border transition-all duration-700 h-full flex flex-col justify-between overflow-hidden cursor-default ${pkg.isPremium ? 'bg-[var(--bg-primary)] text-[var(--text-primary)] shadow-2xl border-[#3A86FF]/50 shadow-[#3A86FF]/10' : 'bg-[var(--bg-secondary)] text-[var(--text-primary)] border-[var(--border-subtle)] hover:border-[#3A86FF]/30'}`}>
                ...
                <div className="relative z-10">
                            ...
                            {pkg.sessionCount.includes('+') ? (
                                <div className="flex flex-col items-end">
                                    <div className="flex items-baseline gap-1">
                                        <span className="text-5xl font-bold font-display text-[var(--text-primary)] leading-none">{pkg.sessionCount.split('+')[0]}</span>
                    ...
                    <div className="space-y-4 mb-12">
                        {pkg.features.map((f, idx) => (
                            <div key={idx} className="flex items-center gap-4 text-[10px] font-bold uppercase tracking-wider text-[var(--text-secondary)]">
                ...
                <div className="relative z-10 mt-auto pt-8 flex flex-col gap-4 border-t border-[var(--border-subtle)]">
                    <div className="mb-6">
                        <div className="flex items-baseline gap-2">
                            <span className="text-6xl font-bold font-display text-[var(--text-primary)]">{pkg.price}</span>
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        <a
                            ...
                            target="_blank"
                            className="flex items-center justify-between p-5 border border-[var(--border-subtle)] text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:border-[#3A86FF]/50 transition-all duration-500"
                        >
