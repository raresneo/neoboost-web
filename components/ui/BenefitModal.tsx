...
export const BenefitModal: React.FC<BenefitModalProps> = ({ isOpen, onClose, benefit }) => {
    ...
    return ReactDOM.createPortal(
        ...
        >
            <div
                ref={modalRef}
                className="bg-zinc-900 text-white border border-[#3A86FF]/30 w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-3xl relative shadow-[0_0_50px_rgba(58,134,255,0.15)] flex flex-col md:flex-row"
            ...
            >
                ...
                <button
                    onClick={onClose}
                    className="absolute top-4 right-4 z-20 bg-zinc-800 p-2 rounded-full text-zinc-400 hover:text-white md:hidden"
                >
                    <X size={20} />
                </button>
                ...
                <div className="flex-1 p-6 md:p-10 text-left relative">
                    <button
                        onClick={onClose}
                        className="absolute top-6 right-6 text-zinc-400 hover:text-white transition-colors hidden md:block"
                    >
                        <X size={24} />
                    </button>
                    ...
                    <h3 className="text-2xl md:text-4xl font-display font-bold text-white uppercase mb-4 leading-tight" style={{ color: '#FFFFFF' }}>
                        {benefit.title}
                    </h3>

                    <p className="text-zinc-300 text-lg leading-relaxed mb-8 font-light" style={{ color: '#d4d4d8' }}>
                        {benefit.content?.intro || benefit.desc}
                    </p>
                    ...
                    <div className="space-y-6 mb-10">
                        ...
                        {/* Mechanisms */}
                        {benefit.content?.mechanisms && (
                            <div>
                                <h4 className="text-white font-bold uppercase tracking-wider text-sm mb-3 opacity-70" style={{ color: '#FFFFFF' }}>Mecanisme de Acțiune</h4>
                                <ul className="space-y-3">
                                    {benefit.content.mechanisms.map((mech, i) => (
                                        <li key={i} className="flex gap-3 text-zinc-300 text-sm items-start" style={{ color: '#d4d4d8' }}>
                        ...
                        {benefit.content?.expectations && (
                            <div className="flex gap-4 items-start p-4 rounded-xl bg-zinc-800 border border-zinc-700">
                                ...
                                <div>
                                    ...
                                    <p className="text-zinc-400 text-sm italic" style={{ color: '#a1a1aa' }}>
                                        "{benefit.content.expectations}"
                                    </p>
                                </div>
                            </div>
