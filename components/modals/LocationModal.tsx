...
export const LocationModal: React.FC<LocationModalProps> = ({ location, isOpen, onClose }) => {
    ...
    return (
        <div
            className="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-[var(--bg-primary)]/95 backdrop-blur-xl animate-fade-in"
            onClick={(e) => {
                if (isZoomed) setIsZoomed(false);
                else onClose();
            }}
        >
            ...
            {isZoomed && (
                <div
                    className="fixed inset-0 z-[1000] bg-[var(--bg-primary)]/95 flex items-center justify-center p-4 cursor-zoom-out animate-in fade-in duration-200"
                    onClick={(e) => {
                        e.stopPropagation();
                        setIsZoomed(false);
                    }}
                >
                    ...
                    <button
                        ...
                        className="absolute top-6 right-6 p-4 bg-[var(--bg-primary)]/50 hover:bg-[#3A86FF] text-white rounded-full transition-colors"
                    >
                        <X size={32} />
                    </button>
                </div>
            ...
            <div
                className={`relative w-full max-w-7xl max-h-[95vh] overflow-y-auto bg-[var(--bg-primary)] border border-white/10 rounded-3xl shadow-2xl animate-scale-in ${isZoomed ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
            ...
            >
                ...
                <button
                    onClick={onClose}
                    className="absolute top-6 right-6 z-50 p-3 bg-[var(--bg-primary)]/80 backdrop-blur-sm border border-white/10 rounded-full text-white/60 hover:text-white hover:bg-[#3A86FF] hover:border-[#3A86FF] transition-all duration-300 group"
                >
                    ...
                    <div className="grid lg:grid-cols-2 gap-12">
                        ...
                        <div className="space-y-6">
                            ...
                            >
                                ...
                                />
                                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-[var(--bg-primary)]/20 pointer-events-none">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="relative w-full h-[600px] bg-[var(--bg-primary)] rounded-2xl overflow-hidden border border-white/10">
